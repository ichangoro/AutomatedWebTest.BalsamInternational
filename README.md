...
**Install Allure CLI globally (if not already):**
```sh
npm install -g allure-commandline
```
...
- **Terminal Output:**  
  Playwright will display test results in the terminal, showing passed/failed/skipped tests.

- **Allure Report:**  
  After running the tests and generating the report, open `./allure-report` in your browser to view detailed, interactive test results, including step-by-step actions, assertions, and screenshots (if configured).

---

**Note:**  
- Ensure Java is installed and `JAVA_HOME` is set for Allure to work.
- Update the `<repository-url>` with your actual repository link.